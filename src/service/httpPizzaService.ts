import { PizzaService, Franchise, Store, OrderHistory, User, UserList, Menu, Order, Endpoints, OrderResponse, JWTPayload } from './pizzaService';
...
class HttpPizzaService implements PizzaService {
  ...
  async getUsers(page: number = 0, limit: 10, emailFilter: string = '*', nameFilter: string = '*', role: string = ''): Promise<UserList> {
    return await this.callEndpoint(`/api/user?page=${page}&limit=${limit}&email=${emailFilter}&name=${nameFilter}&role=${role}`);
  }
